...
read_matrix:
		...
		mov r9, rax 		;total number of elements in r9
...
_start:					
		...
		mov r12, [n]		;cannot be multiplied
		mov r13, [p]
		cmp r12, r13
		jne .exit
	...
	.loop3:
		...
		push qword[mat1+rax]	;rax is the byte offset
		...
		push qword[mat2+rax]	;rax is byte offset

		pop rsi
		pop rax
		mul rsi

		add rbx, rax 			; c[i][j] += a[i][k]*b[k][j], sum over k
